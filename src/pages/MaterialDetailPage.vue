    ...
    <main class="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8">
      <section class="space-y-6">
        ...
        <article class="rounded-[20px] border border-[#e5e7eb] bg-white p-6 shadow-sm">
          ...
          <div class="mt-6">
            <h2 class="text-lg font-semibold">상세 설명</h2>
            <p class="mt-3 text-sm text-[#475569]">{{ material.description }}</p>
          </div>
        </article>
      </section>
      ...
      <aside class="space-y-6">
        <div class="rounded-[16px] border border-[#e5e7eb] bg-white p-6 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-sm text-[#6b7280]">구매 수량</p>
              <p class="mt-2 text-2xl font-bold">{{ quantity }}</p>
            </div>
            <div class="flex flex-col items-end">
              <p class="text-sm text-[#6b7280]">총 금액</p>
